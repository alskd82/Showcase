...
const logoAnimationTime = "#debug" == window.location.hash ? .1 : 2;
const gestureGuideOpenDelay = "#debug" == window.location.hash ? 0.01 : 0.4;
const gestureGuideCloseDelay = "#debug" == window.location.hash ? 0.02 : 2.5;
...
function gestureGuide_Fn() { // 첫 실행 - 제스쳐 가이드 
    ...
    function guidePlay(){
        ...
        document.querySelectorAll("#appLoading .coach-guide").forEach((elem)=>{ elem.classList.add("active");})
    }
    function guideHide(){
        console.log(gestureGuideCloseDelay)
        window.dispatchEvent(new CustomEvent("SHOWCASE_RESIZE"))
...
function logoAnimation_Fn(){ // 로고 애니메이션 
    gsap.to("#appLoading .line", { scaleX: 1, duration: logoAnimationTime, ease: Quart.easeInOut, stagger: .1 })
    gsap.to("#appLoading .logo", { alpha: 1, duration: .2, delay: logoAnimationTime })
    gsap.to("#appLoading .line", {
        scaleX: 0, duration: logoAnimationTime, ease: Quart.easeInOut,
        stagger: -.1, delay: logoAnimationTime + .2,
        onComplete: appLoadComplete_Fn
    })
}
...
function firstPageShow_Fn() { // cover show //
    const _id = 'intro3'; //cover
    const $firstPage = document.querySelector(`#app .page#${_id}`);
    gsap.set($firstPage, { y: 0 })
    // // $firstPage.setAttribute("data-status", "active")// pageStatusActive_Fn 함수에서 선언하고 있음 
